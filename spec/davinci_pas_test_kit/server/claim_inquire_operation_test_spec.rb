RSpec.describe DaVinciPASTestKit::ClaimInquireOperationTest, :runnable do
  ...
  describe 'when submitting one request' do
    it 'passes if the PA response has a 200 status for Claim/$inquire operation' do
      stub_request(:post, "#{server_endpoint}/Claim/$inquire")
        .with(
          body: JSON.parse(pa_inquire_request_payload)
        )
    ...
    it 'fails if the PA response has a status other than 200 or 201 for Claim/$inquire operation' do
      stub_request(:post, "#{server_endpoint}/Claim/$inquire")
        .with(
          body: JSON.parse(pa_inquire_request_payload)
        )
  ...
  describe 'when submitting multiple requests' do
    it 'makes all requests even if some fail' do
      stub = stub_request(:post, "#{server_endpoint}/Claim/$inquire")
        .to_return(status: 400, body: FHIR::Bundle.new.to_json)

      result =
        run(test,
            pa_inquire_request_payload: "[#{pa_inquire_request_payload},#{pa_inquire_request_payload}]",
            server_endpoint:)
      expect(result.result).to eq('fail')
