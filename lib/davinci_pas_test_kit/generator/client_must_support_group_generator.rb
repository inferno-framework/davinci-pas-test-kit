module DaVinciPASTestKit
  class Generator
    class ClientMustSupportGroupGenerator
      class << self
        def generate(ig_metadata, base_client_output_dir)
          ...
          ['submit_must_support', 'inquiry_must_support'].each do |use_case|
            new(ig_metadata, use_case, groups, base_client_output_dir, 'client').generate
          end
        end
      end
      ...
      def title
        if system == 'client'
          return 'Submit Request Must Support' if use_case.include?('submit')

          return 'Inquiry Request Must Support'
        end
      ...
      def alt_request_input_title(request_type)
